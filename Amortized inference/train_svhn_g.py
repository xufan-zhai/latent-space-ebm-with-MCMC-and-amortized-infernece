...
def parse_args():
    ...
    parser.add_argument('--n_metrics', type=int, default=20, help='fid each n epochs')
...
class _netG(nn.Module):
    def __init__(self, args):
        ...
        #generator part
        self.conv1=nn.ConvTranspose2d(args.nz, args.ngf*8, 4, 1, 0, bias = not args.g_batchnorm)
        self.batchnorm=nn.BatchNorm2d(args.ngf*8) if args.g_batchnorm else nn.Identity()
        self.conv2=nn.ConvTranspose2d(args.ngf*8, args.ngf*4, 4, 2, 1, bias = not args.g_batchnorm)
        #nn.BatchNorm2d(args.ngf*4) if args.g_batchnorm else nn.Identity(),
        self.conv3=nn.ConvTranspose2d(args.ngf*4, args.ngf*2, 4, 2, 1, bias = not args.g_batchnorm)
        #nn.BatchNorm2d(args.ngf*2) if args.g_batchnorm else nn.Identity(),
        self.conv4=nn.ConvTranspose2d(args.ngf*2, args.nc, 4, 2, 1)
        self.tanh=nn.Tanh()

        #encoder part
        self.conv5=nn.Conv2d(args.nc, args.ngf, 3, 1, 1, bias=False)#32*32
        self.leakyrelu=nn.LeakyReLU(0.2, inplace=True)
        self.conv6=nn.Conv2d(args.ngf , args.ngf * 2, 4, 4, 1,bias=False)#8*8
        #nn.BatchNorm2d(ngf * 2)
        #nn.LeakyReLU(0.2, inplace=True)
        self.conv7=nn.Conv2d(args.ngf * 2, args.ngf * 4, 4, 2, 1,bias=False)#4*4
        #nn.BatchNorm2d(args.ngf * 4)
        #nn.LeakyReLU(0.2, inplace=True)
        self.conv71 = nn.Conv2d(args.ngf*4, args.nz, 4, 1, 0)
        self.conv72 = nn.Conv2d(args.ngf*4, args.nz, 4, 1, 0) # for log_sigma
    def decode(self,z):
        x=self.conv1(z)
        x=self.batchnorm(x)
        x=self.leakyrelu(x)
    ...
    def encode(self,x):
        ...
        z=self.batchnorm(z)
        z=self.leakyrelu(z)
        return self.conv71(z), self.conv72(z)        
...
def train(args_job, output_dir_job, output_dir, return_dict):
    ...
    def sample_langevin_post_z(mu,log_var,normal):
        std = torch.exp(0.5*log_var)
        return normal.mul(std).add_(mu),0,0
        # z = z.clone().detach()
        # z.requires_grad = True
    ...
    def diag_normal_LL(z, z_mu, z_log_sigma):
        ...
        nll = -0.5 * torch.sum(z_log_sigma.squeeze(), dim=1) - \
              0.5 * torch.sum((torch.mul(z - z_mu, z - z_mu) / (1e-8 + torch.exp(z_log_sigma))).squeeze(), dim=1)
        return nll.squeeze()

    def diag_standard_normal_LL(z):
    ...
    def get_fid(n):
        ...
        try:
            ...
            def sample_x():
                ...
                x_samples = to_range_0_1(netG.decode(z_k)).clamp(min=0., max=1.).detach().cpu()
                return x_samples
    ...
    for epoch in range(args.n_epochs):
        ...
        for i, (x, y) in enumerate(dataloader_train, 0):
            ...
            _,mu_i,log_sigma_i=netG(x)
            z_g_k=repara(mu_i,log_sigma_i,z_g_0)
            z_g_grad_norm, z_e_grad_norm= 0,0
            ...
            # optI.zero_grad()
            # E_T = energy(netE(z_g_k.detach())).mean()
            # x_hat_I=netG(z_g_k.detach())
            # errRecon = mse(x_hat_I, x) / batch_size              
            # log_p_z = torch.mean(diag_standard_normal_LL(z_g_k))  # [batch]--> scalar
            # log_q_z = torch.mean(diag_normal_LL(z_g_k, mu_i, log_sigma_i))  # [batch]---> scalar
            # #print("test!")
            # #print(math.isnan(log_q_z))
            # #errKld: KL[q(z|x)|p(z)]
            # #errKld = log_q_z - log_p_z-E_T
            # errKld = torch.mean(-0.5 * torch.sum(1 + log_sigma_i - mu_i ** 2 - log_sigma_i.exp(), dim = 1), dim = 0)
            # errI = errKld+errRecon-E_T
            # errI.backward()
            # torch.nn.utils.clip_grad_norm(netI.parameters(), 10)
            # optI.step()


            # Learn generator
            optG.zero_grad()
            x_hat,mu_i,log_sigma_i = netG(x)
            recon_err = mse(x_hat, x) / batch_size
            errKld = torch.mean(-0.5 * torch.sum(1 + log_sigma_i - mu_i ** 2 - log_sigma_i.exp(), dim = 1), dim = 0)
            z_g=torch.rand_like(log_sigma_i)
            z_post=repara(mu_i,log_sigma_i,z_g)
            E_T = energy(netE(z_post.detach())).mean()
            loss_g=recon_err+errKld-E_T
            loss_g.backward()
            ...
            if i % args.n_printout == 0:
                with torch.no_grad():
                    x_0 = netG.decode(z_e_0)
                    x_k = netG.decode(z_e_k)
                    ...
                    logger.info('{} {:5d}/{:5d} {:5d}/{:5d} '.format(job_id, epoch, args.n_epochs, i, len(dataloader_train)) +
                        'loss_g={:8.3f}, '.format(loss_g.item()) +
        ...
        if epoch % args.n_stats == 0:
            ...
            stats['z_g_grad_norm'].append(z_g_grad_norm)
            stats['z_e_grad_norm'].append(z_e_grad_norm)
        ...
        if epoch % args.n_plot == 0:
            ...
            mu_p,sigma_p=netG.encode(x_fixed)
            
            
            z_g_k=repara(mu_p,sigma_p,z_g_0)
            z_g_grad_norm, z_e_grad_norm= 0,0
            ...
            with torch.no_grad():
                ...
                plot('{}/samples/{:>06d}_{:>06d}_x_z_pos.png'.format(output_dir, epoch, i), netG.decode(z_g_k))
                plot('{}/samples/{:>06d}_{:>06d}_x_z_neg_0.png'.format(output_dir, epoch, i), netG.decode(z_e_0))
                plot('{}/samples/{:>06d}_{:>06d}_x_z_neg_k.png'.format(output_dir, epoch, i), netG.decode(z_e_k))
                plot('{}/samples/{:>06d}_{:>06d}_x_z_fixed.png'.format(output_dir, epoch, i), netG.decode(z_fixed))
